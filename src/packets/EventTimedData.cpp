#include "packets/EventTimedData.h"
#include "Session.h"
#include "Logging.h"

#include <stdint.h>

EventTimedDataPacket::EventTimedDataPacket(char hdr, EventType event_type, uint32_t timestamp, uint8_t data) : event_type(event_type), timestamp(timestamp), data(data), Packet(PacketType::EVENT_TIMED_DATA, hdr) {}

void EventTimedDataPacket::process() {
    switch (event_type) {
        // System Events
        ...
        */
        ...
        default:
            //logEvent("Unknown event: " + std::to_string(static_cast<int>(event_type)));
            break;
    }
    return;
}
