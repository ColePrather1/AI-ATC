...
void ManualControlMode::manual_send_features() {
        ...
        throttle_val_read = map(SDL_GameControllerGetAxis(GameController::controller, SDL_CONTROLLER_AXIS_TRIGGERRIGHT), -511, 512, 0, 180);           // Throttle
        elevator_val_read = map(SDL_GameControllerGetAxis(GameController::controller, SDL_CONTROLLER_AXIS_LEFTY), -511, 512, 0, 180);           // Elevator
        rudder_val_read = map(SDL_GameControllerGetAxis(GameController::controller, SDL_CONTROLLER_AXIS_LEFTX), -511, 512, 0, 180);           // Rudder
        ...
        uint8_t right_x = (uint8_t)SDL_GameControllerGetAxis(GameController::controller, SDL_CONTROLLER_AXIS_RIGHTX);   // Roll 
        uint8_t right_y = (uint8_t)SDL_GameControllerGetAxis(GameController::controller, SDL_CONTROLLER_AXIS_RIGHTY);   // Flap / Slat
